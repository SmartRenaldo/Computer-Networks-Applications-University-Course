...
public class DistanceVector {
    ...
    private static List<DVEdge> EDGE_LIST = new ArrayList<>();
    private static final Pattern PATTERN = Pattern.compile("^[-\\+]?[\\d]*$");

    /**
     * store router name and adjacent edges
     **/
    private static final List<DVNode> NODE_LIST = new ArrayList<>();
            ...
            if (s == null || s.trim().length() == 0) {
                ...
                while (true) {
                    ...
                    if (split.length == 3 && ROUTER_NAME_LIST.contains(split[0]) && ROUTER_NAME_LIST.contains(split[1]) && isNumber(split[2])) {
                        EDGE_LIST.add(new DVEdge(split[0], split[1], Integer.parseInt(split[2])));
                        break;
                    } else {
            ...
            if (routerName == null || routerName.trim().length() == 0) {
                ...
                while (true) {
                    if (isLetter(routerName)) {
                        NODE_LIST.add(new DVNode(routerName));
    ...
    private static void update() {
        while (true) {
            ...
            for (int i = 0; i < NODE_LIST.size(); i++) {
                ...
                for (int j = 0; j < NODE_LIST.size(); j++) {
                    if (i != j) {
                        DVNode nodeMessageToBeExtracted = NODE_LIST.get(j);
                            ...
                            for (int l = 0; l < nodeArr[k].length; l++) {
                                if (k != nodeIndex && l != nodeIndex) {
                                    if (k == nodeMessageToBeExtractedIndex && l != nodeMessageToBeExtractedIndex) {
                                        ...
                                        int minCostOfOneLineInTrimedArr = getMinCostOfOneLineInTrimedArr(trimedArr, index);
    ...
    private static int getMinCostOfOneLineInTrimedArr(int[][] trimedArr, int index) {
    ...
    private static boolean getEdgeInfo() {
        ...
        for (DVEdge dvEdge : EDGE_LIST) {
            String router1Name = dvEdge.getRouter1Name();
            String router2Name = dvEdge.getRouter2Name();
            int router1Index = 0;
            int router2Index = 0;
            for (int j = 0; j < NODE_LIST.size(); j++) {
            ...
            table[router1Index][router2Index] = dvEdge.getDistance();
            table[router2Index][router1Index] = dvEdge.getDistance();
        }
...
class DVEdge {
    ...
    public DVEdge(String router1Name, String router2Name, int distance) {
    ...
    public int getRouter1Index(List<DVNode> nodeList) {
        int index = 0;

        for (DVNode dvNode : nodeList) {
            if (dvNode.getRouterName().equals(router1Name)) {
                index = dvNode.getNodeIndex();
            }
        }
    ...
    public int getRouter2Index(List<DVNode> nodeList) {
        int index = 0;

        for (DVNode dvNode : nodeList) {
            if (dvNode.getRouterName().equals(router2Name)) {
                index = dvNode.getNodeIndex();
            }
        }
...
class DVNode {
    ...
    public int[][] getTrimedArr() {
        ...
        for (int i = 0; i < trimedArr.length; i++) {
            System.arraycopy(effiArr, 0, trimedArr[i], 0, trimedArr[i].length);
        }
    ...
    public DVNode(String routerName) {
        this.routerName = routerName;
    }

    public DVNode(String routerName, List<String> neighborList, List<String> destinationList, int[][] arr) {
        this.routerName = routerName;
